...
## 方針確定 (2026-06-14) — option (a) 採用 / Step2 実装済み

- 2段組 (0-71fr / 72-143fr) は**維持**。横長になっても1段化しない。
- ACTION/CELL の実使用列数が初期(7列)を超えたら、列幅を圧縮せず
  用紙幅 (WIDTH_MM) を右に拡張。SOUND/CAMERA は当面固定列数前提。
- 設定 `settings.draw.paperAutoExpand` (既定 ON) で OFF 可能。
- 列数が初期以下なら `getStandardA3PaperWidthMm()` は 297 を返し従来描画と完全一致。

### v0.32.0 実装範囲 (Step2 + 周辺)
- `getStandardA3PaperWidthMm()`: 標準列幅を固定し、超過分で WIDTH_MM を算出。
- `renderTemplate`: 標準A3パスのみ WIDTH_MM を一時拡張 (try/finally で復帰、
  外部テンプレ時は拡張しない)。`drawTimelineColumn` の unitW 式は不変で、
  bodyW 拡大により列幅が標準値で維持される (圧縮回避)。
- ヘッダー/Direction/BOOK は WIDTH_MM ベースのため自動追従。
- Preview fit/zoom・PNG/JPG/PSD 書き出しは canvas 幅追従。画像書き出しの
  サイズヒント表示も拡張幅を反映。
- 設定パネルにトグル追加 (i18n ja/en)。

### 残課題 (後続)
- 手書きレイヤー座標の検証 (用紙拡張は右伸びのみなので左寄せ既存座標は不変の想定だが要実機確認)。
- SOUND/CAMERA を拡張基準に含めるか。
- 印刷時の用紙サイズ注意表示。

## (旧) 最大の論点 (実装前に要決定 — option a で決着)
